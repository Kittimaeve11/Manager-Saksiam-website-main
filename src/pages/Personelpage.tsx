...
const Personelpage = () => {
  ...
  const [confirmDelDialog, setConfirmDelDialog] = useState({
    isOpen: false,
    isLoading: false,
    onConfirm: () => { }
  });
  ...
  const handleMoveItemClick = async (personnelID: number) => {
    ...
    setConfirmDelDialog({
      ...
      isLoading: false,
      onConfirm: async () => {
        setConfirmDelDialog(prev => ({ ...prev, isLoading: true }));
        setLoading(true);
      ...
      >
        ...
        <Box sx={{ px: 3 }}>
          <ComponentsUserTableView
            ...
            handleResetPasswordItemClick={handleMoveItemClick}
