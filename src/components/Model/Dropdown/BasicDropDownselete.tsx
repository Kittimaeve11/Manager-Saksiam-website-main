...
const BasicDropDownselete: React.FC<BasicDropDownseleteitem> = ({
  ...
  return (
    ...
    >
      ...
      >
        <MenuItem value="0">
          <Box sx={{ display: 'flex', alignItems: 'center', color: error ? theme.palette.error.main : theme.palette.mode === 'dark' ? theme.palette.grey[600] : theme.palette.grey[400], fontSize: theme.typography.body2.fontSize }}>
            เลือก{titlename}
          </Box>
        </MenuItem>
        {data.map((product) => (
