...
interface Props {
    ...
    setDuplicateMap: (data: { [key: string]: boolean }) => void
    setNotify: any
}
...
const BoxUploadCSV: React.FC<Props> = ({ setCsvData, setDuplicateMap, setNotify }) => {
    ...
    const parseCSV = (text: string) => {
        ...
        return dataRows.map(row => {
            ...
            headers.forEach((h, i) => {
                ...
                if (key) obj[key] = cols[i]?.replace(/^"|"$/g, "").trim();
            });
    ...
    const checkDuplicate = async (data: any[]) => {
        try {
            const response = await apiFetch(
                `/api/auther/branch/check-duplicate`, // ✅ ต้องมี /branch/
                {
                    method: "POST",
